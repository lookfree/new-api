...
*/
import { Shield, Send, Link2, Unlink } from 'lucide-react'
...
// ============================================================================
// Account Bindings Card Component
// ============================================================================
...
type DialogKey = 'wechat' | 'telegram'
...
/**
 * Third-party sign-in bindings. The email binding lives in the profile card, so
 * this card only appears when at least one other provider is available.
 */
export function AccountBindingsCard({
  ...
  if (bindings.length === 0 && !hasCustomProviders) return null

  return (
    <ProfileCard
      title={t('Account Bindings')}
      description={t('Link accounts you can use to sign in.')}
    >
      <div className='grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3'>
        {bindings.map((binding) => {
          const actionLabel = binding.isBound ? t('Bound') : t('Bind')
              ...
              <div className='flex min-w-0 items-center gap-2.5 sm:gap-3'>
                ...
                <div className='min-w-0'>
                  <div className='flex items-center gap-1.5'>
                    ...
                    {binding.isBound && (
                      <Badge variant='success'>{t('Bound')}</Badge>
                    )}
                  </div>
      ...
      {customProviders && customProviders.length > 0 && (
        <>
          {bindings.length > 0 && <Separator className='my-4' />}
