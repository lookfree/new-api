...
import { NOTIFICATION_METHODS, NOTIFICATION_SETTINGS_ID } from '../constants'
import { parseUserSettings } from '../lib'
import { toNotificationFormSettings } from '../lib/notification-settings'
import { normalizeNotifyType } from '../lib/notification-summary'
...
export function NotificationSettingsCard({
  ...
  const savedSettings = parseUserSettings(profile.setting)
  const notifyDisabled = savedSettings.notify_disabled === true

  // The saved notification fields as one string: the form re-syncs only when
  // these change, not when other settings in the same JSON do (language,
  // sidebar, the alerts switch), which would throw away unsaved edits.
  const savedFormKey = JSON.stringify(toNotificationFormSettings(savedSettings))
  const savedForm = useMemo(
    () => JSON.parse(savedFormKey) as UserSettings,
    [savedFormKey]
  )
  const [settings, setSettings] = useState<UserSettings>(savedForm)
  ...
  return (
    <ProfileCard
      ...
      description={t('Choose how you are alerted when your balance runs low.')}
